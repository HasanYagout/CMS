        ...
        <div class="bg-white col-lg-3 mt-30 rounded rounded-5">
            <ul class="zSidebar-menu my-5" id="sidebarMenu">
                ...
                @foreach($forums as $forum)
                    <li>
                        <a class="d-flex flex-column fs-12 fw-bold text-primary-color {{ $activeForum->id==$forum->id ? 'active bg-custom-primary-light text-primary-color' : '' }}"
                    ...
                    <div class="chat-messages  rounded" style="max-height: 350px; overflow-y: auto;">
                        @foreach($forums as $chat)
                            ...
                            @if($chat->comment)
                                ...
                                @foreach($chat->comment as $comment)
                                    @if($comment->forum_id==$activeForum->id)
                                        <div
                                            ...
                                            <strong
                                                ...
                                                @else
                                                    {{ $comment->user->instructor->first_name.' '.$comment->user->instructor->last_name }}
                                                @endif
                                            </strong>
                                            <div
