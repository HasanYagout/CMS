        ...
        <x-wrapper title="">
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <a class="nav-link active" id="assignments-tab" data-bs-toggle="tab" href="#class" role="tab" aria-controls="assignments" aria-selected="true">Class</a>
                </li>
                <li class="nav-item" role="presentation">
                    <a class="nav-link" id="quizzes-tab" data-bs-toggle="tab" href="#content" role="tab" aria-controls="quizzes" aria-selected="false">Content</a>
                </li>
                <li class="nav-item" role="presentation">
                    <a class="nav-link" id="contact-tab" data-bs-toggle="tab" href="#assignments" role="tab" aria-controls="contact" aria-selected="false">Assignments</a>
                </li>
                <li class="nav-item" role="presentation">
                    <a class="nav-link" id="contact-tab" data-bs-toggle="tab" href="#quizzes" role="tab" aria-controls="contact" aria-selected="false">Quizzes</a>
                </li>
                <li class="nav-item" role="presentation">
                    <a class="nav-link" id="contact-tab" data-bs-toggle="tab" href="#activities" role="tab" aria-controls="contact" aria-selected="false">Activities</a>
                </li>
                <li class="nav-item" role="presentation">
                    <a class="nav-link" id="contact-tab" data-bs-toggle="tab" href="#grades" role="tab" aria-controls="contact" aria-selected="false">Grades</a>
                </li>
            </ul>
            <div class="tab-content" id="myTabContent">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            @if($activeLecture->zoom_link)
                                <section>
                                    <a style="width: 200px" class="zBtn-two text-center mt-30 m-auto d-block" href="{{ $lecture->zoom_link }}">Join Class</a>
                                </section>
                            @endif
                        </div>
                    </div>
                        ...
                        @foreach($activeLecture->assignments as $assignment)
                            <div class="row mb-5 shadow rounded">
                                <div class="col-lg-6 mb-4 d-flex flex-column gap-3">
                                    ...
                                    <h5 class="text-third-color">Marks: {{ $assignment->marks }}</h5>
                                </div>
                                <div class="col-lg-6">
                                    ...
                                    <form method="post" action="{{ route('student.courses.assignments.store', $assignment->id) }}" enctype="multipart/form-data"
                                        ...
                                        <section class="d-flex flex-column gap-2 mt-5">
                                            <input type="text" name="comment" placeholder="Add your comment" class="form-control mb-2" @if($assignment->submittedAssignments->isNotEmpty()) disabled @endif>
                                            <input type="file" name="assignments[]" multiple class="form-control mb-2" @if($assignment->submittedAssignments->isNotEmpty()) disabled @endif>
                                        </section>
                                        <section class="mt-3">
                                            <button class="btn zBtn-three" @if($assignment->submittedAssignments->isNotEmpty()) disabled @endif>Cancel</button>
                                            <button type="submit" class="btn zBtn-two" @if($assignment->submittedAssignments->isNotEmpty()) disabled @endif>Save</button>
                                        </section>
                                        <section>
                                            @if($assignment->submittedAssignments->isNotEmpty())
                                                @if(is_null($assignment->submittedAssignments->first()->grade))
                                                    <h5>Your Grade: <span class="text-danger"> still waiting </span></h5>
                                                @else
                                                    <h5 class="text-primary-color">Your Grade: {{ $assignment->submittedAssignments->first()->grade }}</h5>
                                                @endif
                                            @endif
                                        </section>
                                    </form>
                ...
                <div class="tab-pane fade" id="quizzes" role="tabpanel" aria-labelledby="contact-tab">
                    ...
                    <a href="{{ route('student.courses.quizzes.show', $quiz->id) }}" class="bg-secondary-color btn d-block m-auto text-white w-25">Start Exam</a>
                    @endif
                </div>
                            ...
                            <section class="d-flex justify-content-around bg-light-blue mb-20 p-20">
                                ...
                                <ul>
                                    @foreach(explode(',', $activity->options) as $option)
                                        <li>
                                            <input type="radio" name="activity_{{ $activity->id }}" value="{{ $option }}"
                                    ...
                                    <div id="answer_{{ $activity->id }}" class="answer mt-3">
                                        @if($activity->studentActivity->isNotEmpty())
                                            @if($activity->studentActivity->first()->correct)
                                                <span class="p-2 rounded-4 text-white bg-secondary-color">Correct</span>
                                            @else
                ...
                <div class="tab-pane fade" id="grades" role="tabpanel" aria-labelledby="contact-tab">
                    ...
                    <section>
                        ...
                        <hr>
                        <div class="row">
                            <x-drop title="assignments" total="{{$totalAssignments}}" submitted="{{$submittedAssignments}}">

                            </x-drop>
                            {{--                        <x-drop title="quizzes" total="{{$totalQuizzes}}" submitted="{{$submittedQuizzes}}">--}}

                            {{--                        </x-drop>--}}
                            <x-drop title="activites" total="{{$totalActivities}}" submitted="{{$submittedActivities}}">
